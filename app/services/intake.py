...
from app.services.reference import BANKS

BANK_NAMES = [b.name for b in BANKS]
BANK_ALIASES = {
    "카카오뱅크": ("카카오뱅크", "카뱅", "카카오 뱅크"),
    "국민은행": ("국민은행", "kb국민", "국민 은행", "kb은행"),
    "신한은행": ("신한은행", "신한 은행"),
    "우리은행": ("우리은행", "우리 은행"),
    "하나은행": ("하나은행", "하나 은행", "케이이비하나"),
}
...
형식: {"answers": {...}, "summary": "..."}
연도가 적히지 않은 날짜는 오늘 날짜를 기준으로 가장 가까운 과거로 해석한다."""
...
KEYWORDS: list[tuple[str, str, tuple[str, ...]]] = [
    ...
    ("q3", "금귀금속외화", ("골드바", "순금", "금을 팔", "금 판매", "귀금속", "외화", "달러", "엔화")),
    ...
    for key, value, needles in KEYWORDS:
        ...
        if any(n in lowered for n in needles):
            if key == "q13":
                ...
                out[key] = value
    banks = [name for name, needles in BANK_ALIASES.items() if any(n in lowered for n in needles)]
    if banks:
        out["q11"] = banks
